...
const ONLINE_BA_QUESTIONS_DIR = process.env.ONLINE_BA_QUESTIONS_DIR || path.join(__dirname, '..', 'RAG Data', 'Online BA Questions');
...
function determineOnlineBATopicArea(filename) {
  ...
  // Online BA Questions specific topic areas based on the files found
...
function printUsage() {
  console.log('Usage: node ingest-online-ba-questions.js [options]');
  console.log('');
  console.log('Options:');
  ...
  console.log('  ONLINE_BA_QUESTIONS_DIR  Directory containing Online BA Questions files (default: RAG Data/Online BA Questions)');
...
async function ingestOnlineBAQuestions() {
  try {
    // Handle help flag
    ...
    console.log('🚀 Starting Online BA Questions ingestion...');
    
    // Log configuration
    if (VERBOSE) {
      console.log('\n⚙️  Configuration:');
      console.log(`   Directory: ${ONLINE_BA_QUESTIONS_DIR}`);
    ...
    // Check if Online BA Questions directory exists
    if (!fs.existsSync(ONLINE_BA_QUESTIONS_DIR)) {
      console.error(`❌ Online BA Questions directory not found: ${ONLINE_BA_QUESTIONS_DIR}`);
      process.exit(1);
    }
    
    // Remove any existing online BA questions to avoid duplicates (unless dry run)
    if (!DRY_RUN) {
    ...
    // Get all online BA questions markdown files
    const files = fs.readdirSync(ONLINE_BA_QUESTIONS_DIR)
      .filter(file => file.endsWith('-faq.md'))
      .sort();
    
    console.log(`📚 Found ${files.length} Online BA Questions markdown files`);
    
    if (files.length === 0) {
      console.log('⚠️  No Online BA Questions files found to process');
      return;
    }
    ...
    for (const file of files) {
      try {
        ...
        const filePath = path.join(ONLINE_BA_QUESTIONS_DIR, file);
    ...
    console.log('\n📊 Online BA Questions Ingestion Summary:');
    console.log(`   Online BA Questions files found: ${files.length}`);
    console.log(`   Online BA Questions files processed: ${processedFiles}`);
    console.log(`   Online BA Questions files failed: ${failedFiles.length}`);
    console.log(`   Online BA Questions chunks generated: ${totalChunks}`);
    console.log(`   Online BA Questions chunks in database: ${finalCount[0].count}`);
    ...
    if (processedFiles === files.length && failedFiles.length === 0) {
      if (DRY_RUN) {
        console.log('\n🎉 Online BA Questions dry run completed successfully!');
        console.log('   Run without --dry-run to perform actual ingestion');
      } else {
        console.log('\n🎉 Online BA Questions ingestion completed successfully!');
      }
    } else {
      console.log('\n⚠️  Online BA Questions ingestion completed with some issues');
      if (processedFiles > 0) {
    ...
    if (!DRY_RUN) {
      console.log('\n🔍 You can now test the system with Online BA Questions like:');
      console.log('   - "How does the online request premises matching process work?"');
      console.log('   - "How do I configure automatic matching settings?"');
      console.log('   - "Where do I add additional victims to food poisoning cases?"');
      console.log('   - "How does gazetteer address integration work?"');
      console.log('   - "What fields are used for premises matching?"');
      console.log('   - "How do I find prosecution outcome information?"');
      console.log('   - "What causes user confusion in complex system areas?"');
    } else {
      console.log('\n📋 Dry run completed. Use the following command to run the actual ingestion:');
      console.log('   node scripts/ingest-online-ba-questions.js');
    }
    
  } catch (error) {
    console.error('❌ Online BA Questions ingestion failed:', error);
...
if (require.main === module) {
  ingestOnlineBAQuestions();
}

module.exports = { ingestOnlineBAQuestions };